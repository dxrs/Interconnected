...
public class LinkRay : MonoBehaviour
{
    ...
    private GameObject[] obstacleNetral;
    private GameObject[] obstacleP1;
    private GameObject[] obstacleP2;

    private void Update()
    {
        obstacleNetral = GameObject.FindGameObjectsWithTag("Obstacle");
        ...
        if (isChangeLinkMethod) 
        {
            ...
            for(int i = 0; i < obstacleP1.Length; i++) 
            {
                RaycastHit2D hitP1 = Physics2D.Linecast(player[0].transform.position,
                    obstacleP1[i].transform.position, layerMask[1]);

                if (hitP1.collider != null) 
                {
                    foreach(GameObject op1 in obstacleP1) 
                    {
                        if (hitP1.collider.tag == playerObstacleTag[0] &&
                           Vector2.Distance(player[0].transform.position,
                           op1.transform.position) >= 5)
                        {
                            Debug.DrawLine(player[0].transform.position,
                            obstacleP1[i].transform.position,
                            Color.red);
                            Debug.Log("Red Line Drawn");
                        }
                        else
                        {
                            Debug.DrawLine(player[0].transform.position,
                            op1.transform.position,
                            Color.green);
                            Debug.Log("Green Line Drawn");
                        }
                    }
            ...
            for (int i = 0; i < obstacleP1.Length; i++) 
            {
                if (Vector2.Distance(player[0].transform.position,
                    obstacleP1[i].transform.position) < 7) 
                {
                    RaycastHit2D hitP1 = Physics2D.Linecast(player[0].transform.position,
                    obstacleP1[i].transform.position, layerMask[1]);
            ...
            */
            ...
            for (int i = 0; i < obstacleP1.Length; i++)
            {
                ...
                RaycastHit2D hitP1 = Physics2D.Linecast(player[0].transform.position,
                       obstacleP1[i].transform.position, layerMask[0]);
            ...
            if (nearestObstacle != null)
            {
                ...
                //Debug.Log($"Green Line Drawn to nearest obstacle at distance: {nearestDistance}");
            }
